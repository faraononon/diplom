<template>
  <div class="pa-4">  
  <h2 class="display-2 mb-4 text-xs-center">Аспекты</h2>
    <v-data-table
      :headers="headers"
      :items="dataFromDB"
      class="elevation-1"
    >
      <template v-slot:items="props">
        <td>{{ props.item.idAspect }}</td>
        <td>{{ props.item.AspectName }}</td>
          ...
          <v-card-text>
            <v-container grid-list-md>
              <v-text-field v-model="form.idAspect" 
                            ...
                            data-vv-name="id"
                            ></v-text-field>
              <v-text-field v-model="form.AspectName" 
...
export default { 
    created() {
        this.$store.dispatch("getData", {type: 'Aspects', data: 'dataFromDB'});
    ...
    computed: {
        ...
        subcriterion() {
            let obj = this.subCriterionInfo;
            let array = [];
            for(let prop in obj) {
                array.push(obj[prop].idSubcriterion);
            }
            return array;
        },
    },
    methods: {
            ...
            this.$validator.validateAll().then(valid => {
              if (valid) {
                ...
                if (this.editedIndex > -1) {
                  this.$store.dispatch('updateRow', {name: 'Aspects', val: this.rowData.idAspect});
                } else {
                  this.$store.dispatch('createNewRow', 'Aspects');
                }
                this.close();
              }
            })
        ...
        deleteRow(item) {
            this.$store.dispatch('deleteRow', {name: 'Aspects', val: item.idAspect});
        }
